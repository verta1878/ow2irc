...
*/
{
    ...
    case TY_POINTER:
        ...
        return( U8 );   /* x64: pointers are 8 bytes. U8 uses single GPR
                         * (RAX, not EDX:EAX pair) on x64 targets.
                         * FIXES: struct field offsets for pointer members.
                         * Previously U4 → struct layout computed 4-byte
                         * offsets for pointer fields → malloc + linked
                         * lists crashed. TypeClassSize[U8] = 8 bytes. */
#else
        return( U4 );
#endif
...
#if _TARGET & _TARG_X64
        return( U8 );   /* x64: far pointers = near pointers (flat model) */
#else
        return( CP );
#endif
    default:
...
*/
{
    ...
    if( length == 4 )
        return( U4 );
    if( length == 8 ) return( U8 );  /* x64: 8-byte values use U8 */
    return( XX );
}
