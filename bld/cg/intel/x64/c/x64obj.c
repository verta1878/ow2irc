...
void X64ObjFini( void )
{
    ...
    int orig_combined_len = 0;
    int code_start_saved = 0;
    int *omap = NULL;
    long rela_eh_off = 0;
     ...
     * ================================================================ */
    {
        ...
        char *is_branch = (char *)calloc(combined_len * 2, 1); /* tracks real branch positions */
        omap = (int *)calloc(combined_len + 1, sizeof(int));
        int nlen = 0;
        int p;

        /* Find first function offset — everything before is const init data */
        orig_combined_len = combined_len;
        int code_start = combined_len;
 /* default: no init data */
        {
            int q;
        ...
        code_start_saved = code_start;
        /* Scan code section with REX expansion */
        for( ; p < combined_len; ) {
            omap[p] = nlen;
            /* Indirect JMP via 32-bit jump table: FF 24 85 disp32
             * Rewrite to: MOV EAX,[RAX*4+disp32]; JMP RAX
             * (loads 4-byte table entry, zero-extends to 64-bit) */
            if( p + 6 < combined_len &&
                code_seg1[p] == 0xFF && code_seg1[p+1] == 0x24 &&
                code_seg1[p+2] == 0x85 ) {
                /* 8B 04 85 disp32 = mov eax, [rax*4+disp32] */
                new_code[nlen++] = 0x8B;
                omap[p+1] = nlen;
                new_code[nlen++] = 0x04;
                omap[p+2] = nlen;
                ...
                /* Copy disp32 */
                omap[p+3] = nlen;
                new_code[nlen++] = code_seg1[p+3];
                omap[p+4] = nlen;
                new_code[nlen++] = code_seg1[p+4];
                omap[p+5] = nlen;
                new_code[nlen++] = code_seg1[p+5];
                omap[p+6] = nlen;
                new_code[nlen++] = code_seg1[p+6];
                /* FF E0 = jmp rax */
                new_code[nlen++] = 0xFF;
                new_code[nlen++] = 0xE0;
                p += 7;
            ...
            /* Short Jcc (0x70-0x7F): adjust displacement inline */
            if( code_seg1[p] >= 0x70 && code_seg1[p] <= 0x7F && p + 1 < combined_len ) {
                int8_t old_disp = (int8_t)code_seg1[p + 1];
                int orig_target = p + 2 + old_disp;

                /* Sanity check: target must be within code section
                 * and the original target byte should be at a plausible
                 * instruction boundary (not inside an immediate/disp field) */
                if( orig_target < code_start || orig_target > combined_len ) {
                    new_code[nlen++] = code_seg1[p++];
                    continue;
                }
                /* Verify target byte is a plausible instruction opcode.
                 * x87 modrm bytes in 0x70-0x7F are followed by SIB (0x24/0x25)
                 * or disp fields. Real Jcc targets are instruction opcodes
                 * (0x48=REX, 0x89/8B=MOV, 0x31=XOR, 0x5x=POP, 0xC3=RET, etc.)
                 * Cross-check: the byte at orig_target should also be at a
                 * position mapped by omap (instruction boundary). Since ALL
                 * positions get omap entries, instead check that the target
                 * byte looks like a real opcode, not a SIB/disp/immediate. */
                {
                    unsigned char tgt_byte = (orig_target < combined_len) ? code_seg1[orig_target] : 0;
                    /* Reject if target byte is SIB (0x24/0x25) */
                    if( tgt_byte == 0x24 || tgt_byte == 0x25 ) {
                        new_code[nlen++] = code_seg1[p++];
                        continue;
                    }
                    ...
                    if( p > 0 && code_seg1[p-1] >= 0xD8 && code_seg1[p-1] <= 0xDF
                      && !(p > 1 && code_seg1[p-2] == 0x84 && code_seg1[p-1] == 0xDB) ) {
                        ...
                        if( tgt_byte == 0x88 || tgt_byte == 0x89 || tgt_byte == 0x8A || tgt_byte == 0x8B ) ok = 1; /* MOV */
                        if( tgt_byte == 0x31 || tgt_byte == 0x33 ) ok = 1; /* XOR */
                        if( tgt_byte == 0xC3 || tgt_byte == 0xC9 ) ok = 1; /* RET/LEAVE */
                        if( tgt_byte == 0xEB || tgt_byte == 0xE9 || tgt_byte == 0xE8 ) ok = 1; /* JMP/CALL */
                        if( tgt_byte == 0x03 || tgt_byte == 0x2B || tgt_byte == 0x29 ) ok = 1; /* ADD/SUB */
                        if( tgt_byte == 0xFF ) ok = 1; /* INC/DEC/CALL/JMP */
                        if( (tgt_byte & 0xF8) == 0xB8 ) ok = 1; /* MOV imm (B8-BF) */
                        if( tgt_byte == 0x83 || tgt_byte == 0x81 ) ok = 1; /* ALU imm */
                        if( tgt_byte >= 0xD8 && tgt_byte <= 0xDF ) ok = 1; /* x87 */
                        if( tgt_byte == 0x0F || tgt_byte == 0x84 || tgt_byte == 0x85 ) ok = 1; /* 2-byte/TEST */
                        if( tgt_byte == 0x39 || tgt_byte == 0x3B || tgt_byte == 0x38 || tgt_byte == 0x3A ) ok = 1; /* CMP */
                        if( tgt_byte == 0xF7 || tgt_byte == 0xF6 ) ok = 1; /* NOT/NEG/MUL/DIV */
                        if( tgt_byte == 0xC7 || tgt_byte == 0xC6 ) ok = 1; /* MOV imm to mem */
                        if( tgt_byte == 0xDD || tgt_byte == 0xDB ) ok = 1; /* FLD/FISTP */
                        if( !ok ) {
                            new_code[nlen++] = code_seg1[p++];
                            continue;
                        }
                ...
                is_branch[nlen] = 1; /* mark as real Jcc */
                new_code[nlen++] = code_seg1[p++]; /* opcode */
                omap[p] = nlen;
                if( orig_target >= 0 && orig_target <= combined_len ) {
                    /* Target might not be in omap yet (forward branch).
                     * For backward branches, omap[target] is set. */
                    if( orig_target <= p ) {
                        int new_target = omap[orig_target];
                        int8_t new_disp = (int8_t)(new_target - (nlen + 1));
                        new_code[nlen++] = (uint8_t)new_disp;
                    } else {
                        new_code[nlen++] = code_seg1[p]; /* keep original, fix later */
                    }
                } else {
                    new_code[nlen++] = code_seg1[p];
                }
            ...
            /* JMP short (0xEB): same inline fix */
            if( code_seg1[p] == 0xEB && p + 1 < combined_len ) {
                int8_t old_disp = (int8_t)code_seg1[p + 1];
                int orig_target = p + 2 + old_disp;
                is_branch[nlen] = 1;
                new_code[nlen++] = code_seg1[p++];
                omap[p] = nlen;
                if( orig_target >= 0 && orig_target <= combined_len && orig_target <= p ) {
                    int new_target = omap[orig_target];
                    new_code[nlen++] = (uint8_t)(int8_t)(new_target - (nlen + 1));
                } else {
                    new_code[nlen++] = code_seg1[p];
                }
                p++;
            ...
            /* CALL rel32 (0xE8) / JMP rel32 (0xE9): inline fix */
            if( (code_seg1[p] == 0xE8 || code_seg1[p] == 0xE9) && p + 4 < combined_len ) {
                is_branch[nlen] = 2; /* mark as real CALL/JMP rel32 */
                int32_t old_disp;
                memcpy(&old_disp, code_seg1 + p + 1, 4);
                new_code[nlen++] = code_seg1[p++]; /* opcode */
                if( old_disp != 0 ) { /* non-zero = intra-module, needs fixup */
                    int orig_target = p + 4 + old_disp; /* p is now at disp byte */
                    if( orig_target >= 0 && orig_target <= combined_len ) {
                        int new_src = nlen + 4; /* after the 4-byte disp */
                        int new_target = (orig_target <= p) ? omap[orig_target] : orig_target; /* approx for forward */
                        int32_t new_disp = new_target - new_src;
                        memcpy(new_code + nlen, &new_disp, 4);
                    } else {
                        memcpy(new_code + nlen, code_seg1 + p, 4);
                    }
                } else {
                    memcpy(new_code + nlen, code_seg1 + p, 4); /* external call, keep 0 */
                }
                omap[p] = nlen; omap[p+1] = nlen+1; omap[p+2] = nlen+2; omap[p+3] = nlen+3;
                nlen += 4; p += 4;
                continue;
            }
        ...
        /* Second pass: fix FORWARD branch displacements.
         * The inline fixup only handled backward branches (target <= p).
         * Forward branches kept their original displacement, which is
         * now wrong after REX insertions shifted the target. */
        {
            int j;
            for( j = 0; j < nlen; j++ ) {
                unsigned char opc = new_code[j];
                /* Short Jcc (0x70-0x7F) or JMP short (0xEB) */
                if( (opc >= 0x70 && opc <= 0x7F) || opc == 0xEB ) {
                    if( j + 1 >= nlen ) continue;
                    if( !is_branch[j] ) continue; /* only fix REAL branches */
                    int orig_j = -1;
                    int k;
                    for( k = 0; k < orig_combined_len; k++ ) {
                        if( omap[k] == j ) { orig_j = k; break; }
                    }
                    if( orig_j < 0 ) continue;
                    int8_t orig_disp = (int8_t)code_seg1[orig_j + 1];
                    int orig_target = orig_j + 2 + orig_disp;
                    /* Skip backward branches — already fixed by inline handler */
                    if( orig_target <= orig_j ) { j++; continue; }
                    if( orig_target >= 0 && orig_target <= orig_combined_len ) {
                        int new_target = omap[orig_target];
                ...
                /* CALL/JMP rel32 (0xE8/0xE9) */
                if( (opc == 0xE8 || opc == 0xE9) && j + 4 < nlen ) {
                    if( is_branch[j] != 2 ) { j += 4; continue; }
                    int32_t cur_disp;
                    memcpy(&cur_disp, new_code + j + 1, 4);
                    if( cur_disp != 0 ) { /* non-zero = intra-module */
                        int orig_j = -1;
                        int k;
                        for( k = 0; k < orig_combined_len; k++ ) {
                            if( omap[k] == j ) { orig_j = k; break; }
                        ...
                        if( orig_j >= 0 ) {
                            int32_t orig_disp;
                            memcpy(&orig_disp, code_seg1 + orig_j + 1, 4);
                            int orig_target = orig_j + 5 + orig_disp;
                            if( orig_target >= 0 && orig_target <= combined_len ) {
                                int new_target = omap[orig_target];
        ...
        free(new_code);
        free(is_branch);
        /* omap freed after reloc emission */
    }
        ...
        for( int f = 0; f < fixup_count; f++ ) {
            ...
            if( fixups[f].ext_idx >= 0 ) {
                ...
                }
                {
                    ...
                    /* Also treat as absolute (32S):
                     * - FF = JMP/CALL indirect (jump table ref)
                     * - Fixup in init data region (before code)
                     * - SIB bytes 0x85/0x24/0x25 (indirect addressing) */
                    if( (prev >= 0xB8 && prev <= 0xBF) || prev == 0x68
                      ...
                      || off < (unsigned)code_start_saved ) {
                         ...
                         * to account for REX expansion shifting code positions */
                        int32_t adj_within = within;
                        if( anchor == SEC_TEXT && omap != NULL &&
                            within >= 0 && within <= orig_combined_len ) {
                            adj_within = omap[within];
                        }
                        SET64(rela.r_addend, (int64_t)base + adj_within);
                    } else {
