...
static char *setTargetSystem( OPT_STORAGE *data )
{
    ...
    if( target_name != NULL ) {
        if( strcmp( target_name, "DOS" ) == 0 ) {
            ...
            TargetSystem = TS_LINUX64;  /* openwatcom2irc: x86_64 ELF64 target */
...
static void SetFinalTargetSystem( OPT_STORAGE *data, char *target_name )
{
    ...
    case TS_UNIX:
        PreDefine_Macro( "__UNIX__" );
        break;
    case TS_LINUX64:    /* openwatcom2irc: x86_64 Linux ELF64 target */
        PreDefine_Macro( "__X86_64__" );
        PreDefine_Macro( "__LP64__" );
        PreDefine_Macro( "__amd64__" );
        PreDefine_Macro( "__LINUX__" );
        PreDefine_Macro( "__UNIX__" );
        /* Use DOS code generation (standard 386 codegen produces valid x64 code).
         * Post-processing in X64ObjFini converts OMF→ELF64.
         * Do NOT set any TargetSwitches bits — they change codegen! */
        TargetSystem = TS_DOS;
        break;
#endif
    default:
